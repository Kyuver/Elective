...
# Count job title occurrences (demand/popularity)
job_counts = df['job_title'].value_counts()

# Compute average salary per job title
avg_salaries = df.groupby('job_title')['salary_in_usd'].mean()

# Combine counts and averages for comprehensive analysis
...
# Sort by demand (count) for most in-demand analysis
job_summary_by_demand = job_summary.sort_values(by='demand_count', ascending=False)

# Sort by salary for highest paying analysis
...
# 1. Top 15 highest paying countries with their premium roles
...
# 2. Top 10 Countries by Average Salary
